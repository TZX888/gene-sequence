...
# Gfish_dna = pd.read_csv('LSTM/data/1.csv')
# Hfish_dna = pd.read_csv('LSTM/data/0.csv')
...
# with open('LSTM/data/fish.fasta', 'w') as f:
#     for i in tqdm(Gfish_dna['data']):
#         for item in Kmers_funct(i):
#             f.write(item+","+"1"+"\n")
#     for i in tqdm(Hfish_dna['data']):
#         for item in Kmers_funct(i)[:-1]:
#             f.write(item+","+"0"+"\n")
#     f.write(Kmers_funct(i)[-1]+","+"0")

#数据编码
data_y=[]
data_x=[]
with open("/home/u2208283040/tzx/LSTM/data/fish.fasta") as f:
    for line in tqdm(f):
        line=line.rstrip('\n').split(',')
        x=list(line[0])
        if line[1]=="1":
